...
class KReward(nn.Module):
	...
	def get_return(self, traj):
		if len(traj.shape) == 2:
			traj = einops.rearrange(traj, 'h t -> 1 h t')
		if "walker2d" in self.env_name or "halfcheetah" in self.env_name:
			reward = traj[:, 1:, self.action_dim+8].mean()# / self.dt
			reward_ctrl = - 0.1 * torch.square(traj[:, :-1, :self.action_dim]).sum(-1).mean() * 0.0
		elif "hopper" in self.env_name:
			assert traj.shape[-1] == 14
			reward = traj[:, 1:, self.action_dim+5].mean()# / self.dt
			reward_ctrl = -1e-3 * torch.square(traj[:, :-1, :self.action_dim]).sum(-1).mean() * 0.0
		else:
			assert False
			
		return reward + reward_ctrl