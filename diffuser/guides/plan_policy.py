...
class Policy:
    ...
    def __init__(self, diffusion_model, reward_model, normalizer, reward_guidance_scale=1.):
    ...
    def _reward_guidance(self, traj):
        with torch.enable_grad():
            ## shape: (batch_size*horizon)  * (action_dim+state_dim)
            traj = traj.detach().requires_grad_(True)
            re = self.reward_model.get_return(traj)
            
            return torch.autograd.grad(re, traj)[0] * self.reward_guidance_scale
    ...
    def __call__(self, conditions, debug=False, batch_size=1):
        ...
        sample = self.diffusion_model(conditions, reward_guidance=self._reward_guidance)
